	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"account:signIn": "Connect to Y7 Copilot Cloud",
				"account:cloudBenefitsTitle": "Connect to Y7 Copilot Cloud",
				...
				"account:visitCloudWebsite": "Visit Y7 Copilot Cloud",
				"account:remoteControl": "Roomote Control",
				"account:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with Y7 Copilot Cloud",
...
describe("AccountView", () => {
	it("should display benefits when user is not authenticated", () => {
		...
		expect(screen.getByRole("heading", { name: "Connect to Y7 Copilot Cloud" })).toBeInTheDocument()
	...
	it("should display remote control toggle when user has extension bridge enabled", () => {
		...
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with Y7 Copilot Cloud"),
		).toBeInTheDocument()
	})
